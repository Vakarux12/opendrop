...
class AbsArchiveWrite(ArchiveWrite):
    def add_abs_file(self, path, store_path):
        ...
        libarchive-c 5.x compatible: use built-in pathname support
        (old code used ArchiveEntry(None, entry_p) which broke when
        ArchiveEntry API changed to (archive_p, header_codec)).
        """
        return self.add_files(path, pathname=store_path.lstrip('/'))
